 ...
 * Replacer Controller
 *
 * Handles AJAX operations for the Replacer extension, specifically feed reloading.
...
class FreshExtension_replacer_Controller extends Minz_ActionController {
    ...
    public function reloadAction() {
        ...
        if (!Minz_Request::isPost()) {
            Minz_Log::warning('Replacer: Feed reload attempted with non-POST request');
        ...
        if (!$feedId) {
            Minz_Log::warning('Replacer: Feed reload attempted without valid feed_id');
            echo json_encode(['error' => 'Missing or invalid feed_id']);
            exit();
        }

        Minz_Log::notice('Replacer: Starting feed reload for feed ID ' . $feedId);

        try {
            // Verify user is authenticated
            $username = Minz_User::name();
            if (!$username) {
                Minz_Log::warning('Replacer: Feed reload attempted by unauthenticated user');
            ...
            if (!$feed) {
                Minz_Log::warning('Replacer: Feed reload failed - feed ID ' . $feedId . ' not found for user ' . $username);
                echo json_encode(['error' => 'Feed not found']);
                exit();
            }

            Minz_Log::notice('Replacer: Truncating entries for feed ID ' . $feedId);

            // Clear all existing entries from the feed
            $feedDAO->truncate($feedId);

            Minz_Log::notice('Replacer: Triggering feed refresh for feed ID ' . $feedId);

            // Trigger a fresh reload of the feed (will apply current replacement rules)
            FreshRSS_feed_Controller::actualizeFeedsAndCommit($feedId);

            Minz_Log::notice('Replacer: Successfully reloaded feed ID ' . $feedId);
            echo json_encode(['success' => true]);
        } catch (Exception $e) {
            Minz_Log::error('Replacer: Feed reload error for feed ID ' . $feedId . ' - ' . $e->getMessage());
